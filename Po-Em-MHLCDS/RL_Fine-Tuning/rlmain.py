...
class Trainer():
    def __init__(self,
        ...
        self.strategy_dict = label_dict = {'motivational_directives': 2,
                                           'credibility_information': 6,
                                           'reporting_assistance': 8,
                                           'legal_information': 7,
                                           'emotional_support': 3,
                                           'problem_assessment': 0,
                                           'counseling_support': 4,
                                           'no_strategy': 10,
                                           'safety_guidance': 9,
                                           'reassurance': 5,
                                           'confidentiality_assurance': 1}
    ...
    def initialize_classifier_models(self):
        ...
        # model_dict = torch.load(self.pol_classifier_filename)
        # self.politeness_classifier = RobertaForSequenceClassification.from_pretrained('roberta-large', num_labels=self.num_labels)
        # self.counseling_tokenizer = RobertaTokenizer.from_pretrained('roberta-large')
        # self.counseling_classifier.config.problem_type = 'single_label_classification'
        # self.counseling_classifier.load_state_dict(model_dict['state_dict'])
        # self.counseling_classifier = self.counseling_classifier.to(self.device)
    ...
    def random_split_data(self, data):
        ...
        # train_data = [data[idx] for idx in indices[:800]]
        # val_data = [data[idx] for idx in indices[800:]]
        # train_persona = [persona[idx] for idx in indices[:800]]
        # val_persona = [persona[idx] for idx in indices[800:]]
    ...
    def get_pol_pr(self, candidate_dict, gt_pol_str):
        ...
        for idx, ref in enumerate(candidate_dict):
            # tokenizer for both counseling and politeness are same since both uses roberta tokenizer.
            inputs = self.counseling_tokenizer(candidate_dict[ref], padding=True, truncation=True, return_tensors='pt')
                        ...
                        if not self.train_single_model:
                            ...
                            if self.single_model_to_train == 'counselor':
                                if role_ids[turn_num] == 0:
                                    ...
                                    #                                                      return_dict=False)
                                    # past_ = output[1]
                                    # candidates_dict[dial_turn_str] = convert_sentences_to_strings(generated_sequence,
                                    #                                                               self.tokenizer)
    ...
    def train(self):
        ...
        for i in range(self.n_epochs):
            ...
            for batch in pbar:
                ...
                np.save(self.statsfolder + '/' + 'non_rep_scores.npy', np.array(jacc_scores))
